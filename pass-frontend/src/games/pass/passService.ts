import { Client as PassClient, type Game } from './bindings';
...
/**
 * Service for interacting with the Pass game contract
 */
export class PassService {
  private baseClient: PassClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new PassClient({
  ...
  async prepareStartGame(
    ...
    const buildClient = new PassClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
  ...
  async submitProof(
    ...
    if (proof) {
      ...
      // Se não há proof (modo compatibilidade), enviar bytes vazios
      proofBytes = Buffer.from([]);
    }

    console.log('[SubmitProof] Proof length:', proofBytes.length);
...
// Example: const passService = new PassService(contractId);
